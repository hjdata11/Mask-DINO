...
def _gen_gaussian_heatmap(self, imgSize=1024, distanceRatio=3.34):
...
def add_region_character(self, image, target_bbox, regionbox=None):
    ...
    if np.any(target_bbox[0] < real_target_box[0]) or (
        ...
        # if False:
        warped = cv2.warpPerspective(_gen_gaussian_heatmap.copy(), M, (image.shape[1], image.shape[0]))
        ...
        warped = cv2.warpPerspective(_gen_gaussian_heatmap.copy(), _M, (width, height))
        warped = np.array(warped, np.uint8)
...
def convert_coco_polys_to_masks(segmentations, height, width):
    masks = []
    
    for polygons in segmentations:
        mask = np.zeros([height, width], dtype=np.uint8)
        for polygon in polygons:
            mask = add_region_character(mask, polygon)
        if len(mask.shape) < 3:
            mask = mask[..., None]
...
def build_transform_gen(cfg, is_train):
    ...
    augmentation.extend([
        ...
        RandomHueSaturationValue(hue_shift_limit=40, p=0.5),
        RandomBlur(7, 0.5),
    ])
