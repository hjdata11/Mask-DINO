...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            # hj
            for i in range(10):
                if output_proposals.shape[1] >= self.num_queries:
                    break
                else:
                    output_memory = torch.cat([output_memory, output_memory], 1)
                    output_proposals = torch.cat([output_proposals, output_proposals], 1)
        ...
        # #hj
        # prediction_mask = torch.sum(predictions_mask[-1], dim=1)
        # prediction_mask = torch.unsqueeze(prediction_mask, dim=1)
